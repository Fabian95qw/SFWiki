...
public class OpenAIChatCustomBaseURL implements IBaseExecutable
{
	...
	public void execute(IRuntimeEnvironment context) throws Exception
	{
		...
		OpenAiChatComponent OAIC = (OpenAiChatComponent) context.springApplicationContext() //Get Chat Component
				.getBean(OpenAiChatComponent.class);
		log.debug("Building SimpleOpenAI Client"); 
		SimpleOpenAI AI = SimpleOpenAI.builder().apiKey(Key).baseUrl(BaseURL).build(); //Use the Builder with the Custom BaseURL Variable
		log.debug("Sending Request...");
		Response = OAIC.chat(AI, Text, AID); //Create a Request and Save the Response in a string. (Empty if an error occured)
